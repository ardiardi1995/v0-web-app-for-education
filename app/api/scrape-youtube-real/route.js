import { sql } from '@vercel/postgres';

// Mata pelajaran per kelas berdasarkan Kurikulum Merdeka
const SUBJECTS_BY_CLASS = {
  1: ['Matematika', 'Bahasa Indonesia', 'IPAS'],
  2: ['Matematika', 'Bahasa Indonesia', 'IPAS'],
  3: ['Matematika', 'Bahasa Indonesia', 'IPAS'],
  4: ['Matematika', 'Bahasa Indonesia', 'IPAS', 'Bahasa Inggris'],
  5: ['Matematika', 'Bahasa Indonesia', 'IPAS', 'Bahasa Inggris'],
  6: ['Matematika', 'Bahasa Indonesia', 'IPAS', 'Bahasa Inggris'],
  7: ['Matematika', 'IPA', 'IPS', 'Bahasa Indonesia', 'Bahasa Inggris'],
  8: ['Matematika', 'IPA', 'IPS', 'Bahasa Indonesia', 'Bahasa Inggris'],
  9: ['Matematika', 'IPA', 'IPS', 'Bahasa Indonesia', 'Bahasa Inggris'],
  10: ['Matematika', 'Fisika', 'Kimia', 'Biologi', 'Bahasa Inggris'],
  11: ['Matematika', 'Fisika', 'Kimia', 'Biologi', 'Bahasa Inggris'],
  12: ['Matematika', 'Fisika', 'Kimia', 'Biologi', 'Bahasa Inggris'],
};

// Get category for class
function getCategory(kelas) {
  if (kelas <= 6) return 'SD';
  if (kelas <= 9) return 'SMP';
  return 'SMA';
}

// Search YouTube for videos
async function searchYouTubeVideos(query, maxResults = 50) {
  const apiKey = process.env.YOUTUBE_API_KEY;
  if (!apiKey) {
    throw new Error('YOUTUBE_API_KEY not set');
  }

  const url = `https://www.googleapis.com/youtube/v3/search?part=snippet&q=${encodeURIComponent(query)}&maxResults=${maxResults}&type=video&order=relevance&key=${apiKey}`;

  try {
    const response = await fetch(url);
    if (!response.ok) {
      throw new Error(`YouTube API error: ${response.statusText}`);
    }
    const data = await response.json();
    return data.items || [];
  } catch (error) {
    console.error(`Error searching YouTube for "${query}":`, error);
    return [];
  }
}

export async function POST(request) {
  try {
    // Delete existing videos
    await sql`DELETE FROM videos`;
    console.log('[v0] Cleared all videos');

    let totalInserted = 0;

    // For each class and subject combination
    for (const kelas of Object.keys(SUBJECTS_BY_CLASS).map(Number)) {
      const subjects = SUBJECTS_BY_CLASS[kelas];
      const category = getCategory(kelas);

      for (const subject of subjects) {
        // Search keyword: "Matematika Kelas 1", "Fisika Kelas 7", etc
        const keyword = `${subject} kelas ${kelas}`;
        console.log(`[v0] Searching for: ${keyword}`);

        const videos = await searchYouTubeVideos(keyword, 50);

        if (videos.length === 0) {
          console.warn(`[v0] No videos found for: ${keyword}`);
          continue;
        }

        // Insert videos
        for (const video of videos) {
          try {
            await sql`
              INSERT INTO videos (videoid, title, description, thumbnail, category, subject, kelas, createdat)
              VALUES (
                ${video.id.videoId},
                ${video.snippet.title},
                ${video.snippet.description || ''},
                ${video.snippet.thumbnails?.medium?.url || 'https://i.ytimg.com/vi/default/mqdefault.jpg'},
                ${category},
                ${subject},
                ${kelas},
                NOW()
              )
            `;
            totalInserted++;
          } catch (err) {
            console.warn(`[v0] Failed to insert video: ${video.snippet.title}`, err.message);
          }
        }

        console.log(`[v0] Inserted ${Math.min(videos.length, 50)} videos for ${subject} Kelas ${kelas}`);
        
        // Rate limiting: 1 second delay between API calls
        await new Promise(resolve => setTimeout(resolve, 1000));
      }
    }

    console.log(`[v0] Total videos inserted: ${totalInserted}`);

    return Response.json({
      success: true,
      message: `Successfully scraped and inserted ${totalInserted} videos from YouTube`,
      totalVideos: totalInserted,
    });
  } catch (error) {
    console.error('[v0] Scraping error:', error);
    return Response.json(
      {
        error: error.message,
        message: 'Failed to scrape YouTube videos',
      },
      { status: 500 }
    );
  }
}
