...
export async function POST(request) {
  try {
    console.log(`\n[v0] ╔════════════════════════════════════════╗`);
    console.log(`[v0] ║  SCRAPE KELAS 10-12 SMA STARTED        ║`);
    console.log(`[v0] ╚════════════════════════════════════════╝\n`);
    ...
    console.log(`[v0] Deleting old kelas 10-12 data for re-scraping...`);
    await sql`DELETE FROM videos WHERE kelas IN (10, 11, 12)`;
    ...
    for (const kelas of [10, 11, 12]) {
      console.log(`[v0] ▶ Processing KELAS ${kelas} SMA...\n`);
      const subjects = SUBJECTS_BY_CLASS[kelas];
      
      for (const subject of subjects) {
        const query = `${subject} kelas ${kelas} SMA`;
        totalSearched++;
      ...
      console.log(`[v0] ✓ KELAS ${kelas} SMA COMPLETE\n`);
    }

    console.log(`[v0] ╔════════════════════════════════════════╗`);
    console.log(`[v0] ║  SCRAPE SMA SUMMARY                    ║`);
    console.log(`[v0] ╠════════════════════════════════════════╣`);
    ...
    return Response.json({
      success: true,
      message: `Inserted ${totalInserted} videos for kelas 10-12 SMA`,
